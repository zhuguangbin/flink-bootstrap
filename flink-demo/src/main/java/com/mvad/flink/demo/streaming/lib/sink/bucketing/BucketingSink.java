 ...
 */
package com.mvad.flink.demo.streaming.lib.sink.bucketing;
 ...
 * {@link StringWriter} is used, which writes the result
 ...
 * {@link SequenceFileWriter} can be used to write
...
public class BucketingSink<T>
	 ...
	 */
	private final String DEFAULT_PART_SUFFIX = "";
	...
	private void openNewPartFile(Path bucketPath, BucketState<T> bucketState) throws Exception {
		...
		Path partPath = new Path(bucketPath, partPrefix + "-" + subtaskIndex + "-" + bucketState.partCounter + partSuffix);
	...
	/**
	 * Sets the suffix of part files.  The default is empty.
	 */
